          ...
          <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'} animate-in fade-in slide-in-from-bottom-4 duration-300`}>
            <div className={`flex gap-5 max-w-3xl ${msg.role === 'user' ? 'flex-row-reverse' : 'flex-row'}`}>
              ...
              <div className={`p-6 rounded-3xl text-base leading-relaxed shadow-sm overflow-hidden ${msg.role === 'user'
                ...
                : 'bg-gray-50 dark:bg-slate-900 text-gray-800 dark:text-white rounded-tl-none border dark:border-slate-800'
                }`}>
                <div className={`prose dark:prose-invert max-w-none ${msg.role === 'user' ? 'prose-p:text-white prose-headings:text-white prose-strong:text-white prose-li:text-white' : 'dark:prose-p:text-white dark:prose-headings:text-white dark:prose-strong:text-white dark:prose-li:text-white dark:prose-code:text-white'}`}>
