...
const PrepPlanner: React.FC = () => {
    ...
    const loadProgress = async () => {
      ...
      try {
        ...
        if (profile?.questProgress && profile.questProgress.active) {
          ...
          setDays(qp.days);
          setLevel(qp.level);
          setSelectedRole(qp.role); // Sync role from saved progress
          setQuestMeta({
          ...
          // User has a profile role but no active quest - set it as selected
  ...
  const handleGenerateQuest = async () => {
    ...
    setLoading(true);
    try {
      const data = await generatePrepPlan(selectedRole, days, level);
      setQuestData(data);
      setSetupMode(false);
      ...
      alert(`Quest generation failed: ${error.message || 'Unknown error'}. Check console for details.`);

      // Fallback logic kept simple for brevity, same as before
      const fallbackData = {
        questName: `The ${selectedRole} Protocol`,
      ...
      <div className="min-h-[80vh] flex items-center justify-center p-4">
        <div className="w-full max-w-2xl bg-white dark:bg-slate-950 rounded-[2.5rem] p-10 shadow-2xl border dark:border-slate-800 animate-in zoom-in-95 duration-500">
          ...
          <div className="space-y-6">
            <div>
              <label className="block text-sm font-bold text-gray-900 dark:text-white mb-2 ml-1">Target Role</label>
              <select
                value={selectedRole}
